...
import { z } from 'zod'
import { createCustomer, listCustomers, getCustomerById, updateCustomerById, deleteCustomerById } from './customer.service'
...
export default async function customerRoutes(fastify: FastifyInstance) {
    ...
    handler: async (request, reply) => {
      const { name, email, phone, addr_country, addr_city, addr_zip, addr_street, addr_line1, addr_line2, customerType } =
        createCustomerSchema.body.parse(request.body)

      const newCustomer = await createCustomer(name, email, phone, addr_country, addr_city, addr_zip, addr_street, addr_line1, addr_line2, customerType)
